...
$message = "<html><body><h3><b>Script running from:</b></h3><p>" . $scriptpath;
...
//
// Literally no help offered if yuo change this and it doesnt work, just
// as im not flying over to show you how to drive on the correct side of
// the rod. Edit it and you're on your own :)
...
// are evaluted for banning. 
//
//As the IPS log timestamps last for 12+ hours if we used the IPS log we would
// just keep banning the same MAC's over and over. on its original tiemstamp
...
$oui_file = $scriptpath . "ubiquitioui.txt";
...
$log_file = $scriptpath . "autoban.log";

// log reset time - since we're unlikely to want to keep logs, set a reset time - default 24 hours
$log_reset = "24 hours";
...
// reset log if older than $log_reset
if (file_exists($log_file)) {
    if (filemtime($log_file) < strtotime('-' . $log_reset)) {
        unlink($log_file);
    }
}
...
// temp oui file
$oui_file_temp = $scriptpath . "manuf.txt";
...
foreach (array_combine($sites_array, $sites_friendly_array) as $site => $sitefriendly) {
    ...
    $banned_mac_log = $scriptpath . 'bannedmacs_' . $site . '.log';
